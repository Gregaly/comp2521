...
{
	struct IntListNode *previousNode = NULL;
	struct IntListNode *currentNode = NULL;
	struct IntListNode *createdNode = newIntListNode(v);
	...
	for (int i = 0; i < L->size; i++)
	{
		if (currentNode->data > v)
		{
			...
			{
				struct IntListNode *temp = previousNode->next;
				previousNode->next = createdNode;
				createdNode->next = temp;
				L->size++;
			}
			return;
		}
